...
# SubagentStop hook for implementer agents.
# Fires when an implementer finishes. Injects review mandate into orchestrator context.
# Does NOT block the agent from stopping — it injects post-stop context.
...
envsubst '${AGENT_TYPE} ${LAST_MESSAGE}' < "$SCRIPT_DIR/templates/review-mandate.txt"
