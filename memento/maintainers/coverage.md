...
**pat-down.** Both its last findings — the `situation` / `found state` term collision and the missing span-start marker in the journal read — were closed by edits made after both reports. No checker read the shipped text.
...
**schema-resolution.** Its last compliance report's two violations — one sentence carrying two instructions, and a record-keeping instruction with no paired prohibition — were closed by an edit made after the report. No checker read the shipped text. Its shipped-default reference is recorded below.
