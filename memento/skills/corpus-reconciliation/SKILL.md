...
description: >
  Audit every record one project holds against the index over them, then repair each mismatch found.
  "audit the records", "reconcile the index", "rebuild the map", the index and the records drifted apart,
  two records in force contradict each other, a record the index omits, a pointer whose target is gone,
  content carrying no recorded author, a recurring audit timer fires.
---
