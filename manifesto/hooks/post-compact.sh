...
if [ -n "$manifestos" ]; then
    ...
    cat <<EOF
{
  "systemMessage": "MANIFESTO RE-BINDING REQUIRED — context was just compacted.\\n\\nDeclared manifestos:\\n${escaped}\\n\\n${RESOLVE_INSTRUCTIONS}\\n\\nRe-load each manifesto's full text and re-apply the manifesto-oath skill. Previous bindings did not survive compaction. This is non-negotiable."
}
EOF
else
    cat <<EOF
{
  "systemMessage": "MANIFESTO PLUGIN ACTIVE — context was just compacted. If manifestos were previously bound, re-apply the manifesto-oath skill. Repo manifestos available in: ${MANIFESTO_DIR}/manifestos/."
}
EOF
fi
